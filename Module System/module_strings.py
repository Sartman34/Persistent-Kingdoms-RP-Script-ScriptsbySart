...
"),
  ...
  ("your_current_position_is", "Suanki konumun: (x:{reg11}, y:{reg12}, z:{reg13})"),
  ("your_distance_to_position_is", "Senin (x:{reg11}, y:{reg12}, z:{reg13}) pozisyonuna (x:{reg21}, y:{reg22}, z:{reg23}) olan uzakligin: {reg14}m"),
  ("quit", "Quit"),
  ...
  ("log_admin_ban_temp", "temp ban'ladi"), #gecici/kalici yasakladi diyince kafam duruyor. bence boyle kalsin.
  ("log_admin_ban_perm", "perma ban'ladi"),
  ("log_admin_mute", "mute'ladi / kaldirdi"),
  ("log_admin_mute_all", "butun oyunculari mute'ladi"),
  ("log_admin_kill", "slay'ledi"),
  ("log_admin_fade_out", "fade'ledi"),
  ("log_admin_freeze", "freeze'ledi / kaldirdi"),
  ("log_admin_teleport_to", "oyuncuya isinlandi"),
  ("log_admin_teleport_behind", "oyuncunun arkasina isinlandi"),
  ("log_admin_teleport", "isinlandi"), #?
  ("log_admin_teleport_forwards", "ileri isinlandi"),
  ("log_admin_refill_health", "canini yeniledi"),
  ("log_admin_become_godlike", "godlike oldu"),
  ("log_admin_get_armor", "admin zirhi giydi"),
  ("log_admin_get_invisible", "gorunmez oldu"),
  ("log_admin_get_horse", "admin ati basti"),
  ("log_admin_remove_horses", "admin atlarini kaldirdi"),
  ("log_admin_remove_stray_horses", "basibos atlari kaldirdi"),
  ("log_admin_teleport_to_ships", "siradaki gemiye isinlandi"),
  ("log_admin_reset_ships", "batik gemileri yeniledi"),
  ("log_admin_join_faction", "katildi"),
  ("log_admin_lock_faction", "{s4}locked faction {s5}"), #?
  ("log_admin_own_position", "kendi pozisyonuna bakti (x:{reg11}, y:{reg12}, z:{reg13})"),
  ("log_admin_own_distance", "kendinden (x:{reg11}, y:{reg12}, z:{reg13}) pozisyona (x:{reg21}, y:{reg22}, z:{reg23}) olan uzakligina ({reg14}m) bakti"),
  ("log_admin_cheat_money", "kendine {reg0} altin basti"),
  ("log_admin_cheat_item", "kendine '{s4}' itemini basti"),
  ("log_admin_restocked_s4", "'{s4}'un stocklarini yeniledi"),
  ...
  ("admin_chat_format", "*INTERNAL* [{s1}] {s0}"),
  ("admin_chat_player_format", "*PLAYER* [{s1}] {s0}"),
  ("admin_chat_to_player_format", "[{s1}] > [{s2}] {s0}"),
  ("admin_announcement_format", "[{s1}] {s0}"),
...
Poisonous herbs can also sometimes be found, which are then applied to the blades of certain weapons used for murderous deeds; they have many brown stalks covered in tiny green leaves."),
  ...
  #Log formats
  ("attach_to_himself_log", "{s11}, kendisine cart bagladi. (id: {reg31})"),
  ("attach_to_horse_log", "{s11}, atina cart bagladi. (id: {reg31})"),
  ("detach_from_himself_log", "{s11}, cartini kendisinden birakti. (id: {reg31})"),
  ("detach_from_horse_log", "{s11}, cartini atindan birakti. (id: {reg31})"),
  ("log_use_money_bag", "{s11}, {reg31} altin iceren keseyi acti."),
  ("log_spawn_money_bag", "{s11}, {reg31} altin iceren keseyi birakti. (id: {reg0})"),
  ("log_money_chest_withdraw", "{s11}, {s12} faction'unun sandigindan {reg31} altin aldi."),
  ("log_money_chest_deposit", "{s11}, {s12} faction'unun sandigina {reg31} altin koydu."),
  ("log_hit_door", "{s11}, {s12} faction'unun kapisina {reg31} puan hasar verdi. (id: {reg32})."),
  ("log_repair_door", "{s11}, {s12} faction'unun kapisini {reg31} puan tamir etti. (id: {reg32})"),
  ("log_hit_chest", "{s11}, {s12} faction'unun sandigina {reg31} puan hasar verdi. (id: {reg32})"),
  ("log_repair_chest", "{s11}, {s12} faction'unun sandigini {reg31} puan tamir etti. (id: {reg32})"),
  ("log_hit_ship", "{s11}, bir gemiye vurdu. (id: {reg31})"),
  ("log_drop_item", "{s11}, {s12} itemini yere birakti. (id: {reg31})"),
  ("log_pick_up_item", "{s11}, {s12} itemini yerden aldi. (id: {reg31})"),
  ("log_put_item_in_inventory", "{s11}, {s12} itemini bir envantere koydu. (id: {reg31})"),
  ("log_put_item_in_finventory", "{s11}, {s10} faction'unun envanterine {s12} itemini koydu. (id: {reg31})"),
  ("log_take_item_from_inventory", "{s11}, {s12} itemini bir envanterden aldi. (id: {reg31})"),
  ("log_take_item_from_finventory", "{s11}, {s10} faction'unun envanterinden {s12} itemini aldi. (id: {reg31})"),
  ("log_hit_player", "{s11}, {s12} ile {s10} oyuncusuna {reg31} hasar verdi. id'ler: {reg10}, {reg11}"),
  ("log_hit_phorse", "{s11}, {s10} ile ({s12}) atina {reg31} hasar verdi. id'ler: {reg10}, {reg11}"),
  ("log_hit_animal", "{s11}, {s10} ile ({s12} (id: {reg32})) atina {reg31} hasar verdi. id: {reg10}"),
  ("log_bump", "({s11} id: {reg10})) ati, ({s12} id: {reg11}) oyuncusuna {reg31} hasar verdi."),
  ("log_heal_player", "{s11}, {s12} oyuncusunu {reg31} HP iyilestirdi."),
  ("log_heal_phorse", "{s11}, ({s12}) atini {reg31} HP iyilestirdi."),
  ("log_heal_animal", "{s11}, ({s12}) atini {reg31} HP iyilestirdi."),
  ("log_loot_corpse", "{s11}, bir ceset lootladi. (id: {reg31})"),
  ("log_s11_dropped_loot", "{s11}, loot dusurdu. (id: {reg31})"),
  ("log_use_cart", "{s11}, bir cartin icine bakti (id: {reg31})"),
  ("log_capture", "{s9} ({s10}), {s12} faction'unun {s11} kalesini fethetti."),
  ("log_capture_secondary", "{s9} ({s10}), {s12} faction'unun {s11} ikincil noktasini fethetti."),
  #End

  #Shield Log
  ("shield_hit_log", "{s12}, {s10} ile {s11} oyuncusunun kalkanina {reg31} hasar verdi."),
  
  ("log_equipment", "{s11}, bu itemlere sahipti: H:{reg31}, B:{reg32}, G:{reg33}, F:{reg34}, HO:{reg39}, 1:{reg35}, 2:{reg36}, 3:{reg37}, 4:{reg38}"),
  ("s1_kicked_s2_from_s3", "{s1}, {s2} oyuncusunu {s3} faction'undan kick'ledi."),
  ("s1_outlawed_s2_from_s3", "{s1}, {s2} oyuncusunu {s3} faction'unda kanunsuz ilan etti."),#?
  ("s2_kicked_you_from_the_faction", "{s2}, seni faction'undan kick'ledi!"),
  ("s2_outlawed_you_from_the_faction", "{s2} seni faction'unda kanunsuz ilan etti!"),

  ("fist", "yumruk"),
  
  ("time_day", "Time:_Day"),
  ("time_night", "Time:_Night"),
  ("time_sunrise", "Time:_Sunrise"),
  ("time_sunset", "Time:_Sunset"),
  ("time_error", "[E]_Could_not_get_one_or_more_skybox_prop_instances!"),
