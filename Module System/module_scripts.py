...
##    (store_script_param, ":chat_type", 2),
    ...
    (try_begin),
      (str_equals, s0, "@/debug"),
      (try_begin),
        (player_slot_eq, ":player_id", slot_player_is_debug_mode, 0),
        (player_set_slot, ":player_id", slot_player_is_debug_mode, 1),
      (else_try),
        (player_set_slot, ":player_id", slot_player_is_debug_mode, 0),
      (try_end),
    (else_try),
      (str_equals, s0, "@/hit"),
      (try_begin),
        (player_is_admin, ":player_id"),
        (val_add, "$g_is_hit_disabled", 1),
        (val_mod, "$g_is_hit_disabled", 2),
        (try_for_players, ":other_player_id", 1),
          (try_begin),
            (eq, "$g_is_hit_disabled", 0),
            (multiplayer_send_2_int_to_player, ":other_player_id", server_event_script_message_set_color, 4284546209),
            (multiplayer_send_string_to_player, ":other_player_id", server_event_script_message, "@Hitler tekrardan aktif!"),
          (else_try),
            (multiplayer_send_2_int_to_player, ":other_player_id", server_event_script_message_set_color, 4293938509),
            (multiplayer_send_string_to_player, ":other_player_id", server_event_script_message, "@Hitler kapatildi."),
          (try_end),
        (try_end),
      (try_end),
    (else_try),
      (str_starts_with, s0, "@/t "),
      (try_begin),
        (player_is_admin, ":player_id"),
    ...
    (else_try),
      ...
      (try_begin),
        (player_is_admin, ":player_id"),
        (val_add, "$g_is_nlr_closed", 1),
        (val_mod, "$g_is_nlr_closed", 2),
        (try_for_players, ":other_player_id", 1),
          (try_begin),
            (eq, "$g_is_nlr_closed", 0),
            (multiplayer_send_2_int_to_player, ":other_player_id", server_event_script_message_set_color, 4284546209),
            (multiplayer_send_string_to_player, ":other_player_id", server_event_script_message, "@NLR Acildi!"),
          (else_try),
            (multiplayer_send_2_int_to_player, ":other_player_id", server_event_script_message_set_color, 4293938509),
            (multiplayer_send_string_to_player, ":other_player_id", server_event_script_message, "@NLR Kapandi."),
          (try_end),
        (try_end),
        (try_begin),
          (eq, "$g_is_nlr_closed", 0),
    ...
    (else_try),
      (str_starts_with, s0, "@/clear "),
      (try_begin),
        (player_is_admin, ":player_id"),
        (str_split, ":lenght", s0, s0, "str_space", 1),
        (ge, ":lenght", 2),
        (str_is_digit, s1),
        (str_to_num, ":instance_id", s1),
        ...
        (try_for_range, ":inventory_slot", slot_scene_prop_inventory_begin, ":inventory_slot_end"),
          (scene_prop_set_slot, ":instance_id", ":inventory_slot", 0),
        (try_end),
        (try_for_players, ":other_player_id", 1),
    ...
    (else_try),
      ...
      (neg|str_contains, s0, "str_dspace"),
    
      (assign, reg10, ":player_id"),
      (assign, reg11, chat_event_type_local),
      (player_get_unique_id, reg12, ":player_id"),
      (player_get_slot, ":faction_id", ":player_id", slot_player_faction_id),
      (faction_get_color, reg13, ":faction_id"),
...
##      (assign, ":variation_id_2", reg2),
      ...
      (scene_prop_set_slot, ":instance_id", ":slot", ":item_id"),
      (store_add, reg3, ":index", 1),
      (send_message_to_url_advanced, script_ip_address + "/load_chest<{reg1}<{reg2}<{reg3}<{reg4}", "@WSE2", "script_load_chest_return", "script_load_chest_fail"),
    (else_try),
      (eq, ":action", 2),
      (assign, ":scene_prop_id", reg1),
##      (assign, ":variation_id_2", reg2),
  ...
  ("load_player_return", [
    ...
    (try_begin),
      ...
      (try_begin),
        ...
        (faction_get_slot, reg0, ":faction_id", slot_faction_lord_player_uid),
        (this_or_next|eq, reg0, ":unique_id"),
        (neg|dict_has_key, "$g_player_id_dict", "@{reg0}"),
        (call_script, "script_cf_faction_set_lord", ":player_id", ":unique_id", ":faction_id"),
      (try_end),
      ...
      (try_begin),
        ...
        (try_begin),
          ...
          (spawn_scene_prop, "spr_pw_wheelbarrow"),
          ...
          (scene_prop_set_slot, ":instance_id", slot_scene_prop_personal_inventory_player_id, ":player_id"),
...
##      (assign, ":count", reg2),
  ...
  ("special_string_fail", [ (display_message, "@special_string request failed."), ]),

  ("ping_tcp_return", [
    (store_script_param, ":register_count", 1),
...
##      (assign, ":unique_id", reg1),
      ...
      (try_begin),
        (str_equals, s1, s2),
      (else_try),
        (str_store_string, s0, "@Your registered name is: {s1}"),
        (call_script, "script_send_coloured_message", ":player_id", colors["beyaz"], 0),
        (eq, ":kick", 1),
        (kick_player, ":player_id"),
      (try_end),
    (else_try),
      ...
      (try_for_agents, ":agent_id"),
        ...
        (assign, reg1, ":food_amount"),
        (player_is_active, ":player_id"),
        (try_begin),
          (lt, ":food_amount", 2),
          (assign, ":food_amount", 0),
          (multiplayer_send_2_int_to_player, ":player_id", server_event_script_message_set_color, 4288225055),
          (multiplayer_send_string_to_player, ":player_id", server_event_script_message_announce, "@Cok Aciktin!"),
          (store_agent_hit_points, ":hit_points", ":agent_id"),
          #if hit_points - reg2 < 15: set_hit_points(15)
          #(val_sub, ":hit_points", reg2),
          (assign, ":is_sound", 0),
          (try_begin),
            (le, ":hit_points", 25),
          (else_try),
            (assign, ":is_sound", 1),
            (val_sub, ":hit_points", 5),
            (assign, reg10, ":hit_points"),
            (le, ":hit_points", 25),
            (agent_deliver_damage_to_agent, ":agent_id", ":agent_id", 5),
            (agent_set_hit_points, ":agent_id", 25, 0),
          (else_try),
            (agent_deliver_damage_to_agent, ":agent_id", ":agent_id", 5),
...
##      (assign, ":unique_id", reg1),
      ...
      (agent_is_alive, ":agent_id"),
      (try_begin),
        (eq, ":event_type", chat_event_type_local_shout),
...
##      (assign, ":unique_id", reg1),
    ...
    (else_try),
      (eq, ":action", 3),
      (assign, reg0, 1),
      (call_script, "script_special_string_return"),
    (else_try),
      (eq, ":action", 4),
      (assign, ":colour_id", reg1),
      #s0: message
...
##        (assign, ":unique_id", reg2),
    ...
    ("trp_lancer", "spr_pw_change_troop_lancer"),
        ...
        (try_begin),
          ...
          (try_end),
        (else_try),
          (str_store_string, s0, "@No training center nearby."),
          (call_script, "script_send_coloured_message", ":player_id", colors["acik kirmizi"], 1),
        (try_end),
      (else_try),
        (eq, ":command_id", 4),
##        (assign, ":unique_id", reg2),
        (dict_has_key, "$g_player_id_dict", "@{reg2}"),
        (dict_get_int, ":player_id", "$g_player_id_dict", "@{reg2}"),
        (player_get_slot, ":slot", ":player_id", slot_player_army_aggro),
        (val_add, ":slot", 1),
        (val_mod, ":slot", 2),
        (player_set_slot, ":player_id", slot_player_army_aggro, ":slot"),
        (try_begin),
          (eq, ":slot", 0),
          (str_store_string, s0, "@Army relation to commoners is set to passive."),
          (call_script, "script_send_coloured_message", ":player_id", colors["acik yesil"], 0),
        (else_try),
          (eq, ":slot", 1),
          (str_store_string, s0, "@Army relation to commoners is set to aggressive."),
          (call_script, "script_send_coloured_message", ":player_id", colors["koyu kirmizi"], 0),
        (try_end),
        (try_for_agents, ":agent_id"),
...
##        (assign, ":unique_id", reg2),
        ...
        (else_try),
          (eq, ":owner_agent_id", 0),
          (agent_set_no_dynamics, ":horse_agent_id", 1),
          (agent_set_slot, ":horse_agent_id", slot_agent_owner_agent_id, ":agent_id"),
          (str_store_string, s0, "@At bağlandı."),
          (call_script, "script_send_coloured_message", ":player_id", colors["beyaz"], 0),
        (else_try),
          (str_store_string, s0, "@Bu at başkasına ait."),
          (call_script, "script_send_coloured_message", ":player_id", colors["acik kirmizi"], 1),
        (try_end),
      (try_end),
    (else_try),
...
##      (assign, ":unique_id", reg1),
      ...
      (agent_get_position, pos1, ":agent_id"),
      (position_move_z, pos1, 160),
      (assign, ":closest_player_id", 0),
      (assign, ":min_distance", sq(max_distance_local_chat)),
      (try_for_players, ":other_player_id", 1),
        ...
        (position_move_z, pos2, 160),
        (get_sq_distance_between_positions, ":sq_distance", pos1, pos2),
        (le, ":sq_distance", ":ambient_sq_distance"),
        (le, ":sq_distance", ":min_distance"),
        (assign, ":min_distance", ":sq_distance"),
        (assign, ":closest_player_id", ":other_player_id"),
      (try_end),
      (gt, ":closest_player_id", 0),
      (str_store_player_username, s1, ":player_id"),
      (str_store_player_username, s2, ":closest_player_id"),
      (str_store_string, s0, "@[{s1}] whispers [{s2}] {s0}"),
      (call_script, "script_send_coloured_message", ":player_id", ":colour_id", 0),
      (call_script, "script_send_coloured_message", ":closest_player_id", ":colour_id", 0),
    (else_try),
      (eq, ":action", 7),
##      (assign, ":unique_id", reg1),
      (assign, ":colour_id", reg2),
      ...
      (agent_is_active, ":agent_id"),
      (agent_is_alive, ":agent_id"),
      (assign, ":ambient_sq_distance", sq(ambient_distance_local_chat + 150)),
      (set_fixed_point_multiplier, 100),
      (agent_get_position, pos1, ":agent_id"),
      (position_move_z, pos1, 160),
      (str_store_string, s1, s0),
      (str_store_player_username, s2, ":player_id"),
      (str_store_string, s0, "@[{s2}] whispers: {s1}"),
      (call_script, "script_send_coloured_message", ":player_id", ":colour_id", 0),
      (try_for_players, ":other_player_id", 1),
        (neq, ":other_player_id", ":player_id"),
        (player_get_agent_id, ":other_agent_id", ":other_player_id"),
        ...
        (get_sq_distance_between_positions, ":sq_distance", pos1, pos2),
        (le, ":sq_distance", ":ambient_sq_distance"),
        (str_store_player_username, s3, ":other_player_id"),
        (str_store_string, s0, "@[{s2}] whispers [{s3}] {s1}"),
        (call_script, "script_send_coloured_message", ":other_player_id", ":colour_id", 0),
        (str_store_string, s0, "@-> [{s3}]"),
        (call_script, "script_send_coloured_message", ":player_id", ":colour_id", 0),
      (try_end),
    (try_end),
  ]),
...
##    (assign, ":unique_id", reg1),
    ...
    (assign, ":left", reg3),
    (assign, ":is_private", reg4),
    (assign, ":has_access", reg5),
    ...
    (try_begin),
      ...
      (try_begin),
        ...
        (try_end),
      (else_try),
        (assign, reg10, ":is_private"),
        (assign, reg11, ":has_access"),
        (eq, ":is_private", 1),
        (try_begin),
          (eq, ":has_access", 1),
          (multiplayer_send_2_int_to_player, ":player_id", server_event_script_message_set_color, 4288473929),
          (multiplayer_send_string_to_player, ":player_id", server_event_script_message, "@Bu kapi sana ait."),
        (else_try),
          (assign, ":fail_string_id", 5000),
        (try_end),
      (else_try),
      ...
      (else_try),
        (eq, ":fail_string_id", 5000),
        (multiplayer_send_2_int_to_player, ":player_id", server_event_script_message_set_color, 4288225055),
        (multiplayer_send_string_to_player, ":player_id", server_event_script_message_announce, "@Bu kapinin anahtarina sahip degilsin."),
      (try_end),
      (try_begin),
        (player_slot_eq, ":player_id", slot_player_is_debug_mode, 1),
        (assign, reg0, ":instance_id"),
        (multiplayer_send_2_int_to_player, ":player_id", server_event_script_message_set_color, 4294967295),
        (multiplayer_send_string_to_player, ":player_id", server_event_script_message_announce, "@Kilit ID: ({reg0})."),
      (try_end),
      (eq, ":fail_string_id", 0),
  ...
  ("check_door_key_fail", [ (display_message, "@check_door_key request failed."), ]),

  ("check_door_key_tp_return", [ 
    (assign, ":success", reg0),
##    (assign, ":unique_id", reg1),
    (assign, ":instance_id", reg2),
    (assign, ":x_offset", reg3),
    (assign, ":y_offset", reg4),
    (assign, ":z_offset", reg5),
    (assign, ":is_pickable", reg6),
    (assign, ":horse_can_tp", reg7),
    (assign, ":is_private", reg8),
    (assign, ":has_access", reg9),
    ...
    (try_begin),
      ...
      (try_begin),
        ...
        (try_begin),
          (eq, ":has_access", 1),
          (try_begin),
            (scene_prop_slot_eq, ":instance_id", slot_scene_prop_is_cached, 0),
            (troop_get_slot, ":cache_count", "trp_teleport_timer_caches", 0),
            (val_add, ":cache_count", 1),
            (troop_set_slot, "trp_teleport_timer_caches", 0, ":cache_count"),
            (troop_set_slot, "trp_teleport_timer_caches", ":cache_count", ":instance_id"),
            (scene_prop_set_slot, ":instance_id", slot_scene_prop_is_cached, 1),
          (try_end),
          (scene_prop_set_slot, ":instance_id", slot_scene_prop_teleport_timer, 6),
          (multiplayer_send_2_int_to_player, ":player_id", server_event_script_message_set_color, 4288473929),
          (multiplayer_send_string_to_player, ":player_id", server_event_script_message, "@Bu kapi sana ait."),
        (else_try),
      ...
      (try_begin),
        ...
        (assign, reg0, ":instance_id"),
        (multiplayer_send_2_int_to_player, ":player_id", server_event_script_message_set_color, 4294967295),
        (multiplayer_send_string_to_player, ":player_id", server_event_script_message_announce, "@Kapi ID: ({reg0})."),
      (try_end),
      (eq, ":fail_string_id", 0),
  ...
  ("check_door_key_tp_fail", [ (display_message, "@check_door_key request failed."), ]),
  ...
  ("agent_init_relations", [
    ...
    (try_for_agents, ":other_agent_id"),
      ...
      (agent_get_team, ":other_team_id", ":agent_id"),
      (try_begin),
        (teams_are_enemies, ":team_id", ":other_team_id"),
        (agent_add_relation_with_agent, ":agent_id", ":other_agent_id", 1),
        (agent_add_relation_with_agent, ":other_agent_id", ":agent_id", 1),
      (else_try),
        (agent_add_relation_with_agent, ":agent_id", ":other_agent_id", 0),
        (agent_add_relation_with_agent, ":other_agent_id", ":agent_id", 0),
      (try_end),
    (try_end),
    (try_begin),
      (agent_is_human, ":agent_id"),
      ...
      (try_for_agents, ":other_agent_id"),
        ...
        (agent_get_team, ":other_team_id", ":agent_id"),
        (eq, ":other_team_id", team_default),
        (neq, ":other_agent_id", ":agent_id"),
        (try_begin),
          (agent_is_non_player, ":other_agent_id"),
          (agent_get_group, ":other_player_id", ":other_agent_id"),
        (else_try),
          (agent_get_player_id, ":other_player_id", ":other_agent_id"),
          (try_begin),
            (eq, ":is_player", 1),
        ...
        (try_begin),
          ...
          (try_begin),
            (agent_get_troop_id, ":other_troop_id", ":other_agent_id"),
            (eq, ":other_troop_id", "trp_archer"),
            (eq, ":troop_id", "trp_archer"),
            (agent_add_relation_with_agent, ":agent_id", ":other_agent_id", 0),
            (agent_add_relation_with_agent, ":other_agent_id", ":agent_id", 0),
          (try_end),
        (else_try),
          (player_slot_eq, ":player_id", slot_player_army_aggro, 0),
          (this_or_next|eq, ":faction_id", "fac_commoners"),
          (eq, ":other_faction_id", "fac_commoners"),
        (else_try),
          (eq, ":faction_id", "fac_outlaws"),
          (eq, ":other_faction_id", "fac_outlaws"),
        (else_try),
          (is_between, ":faction_id", castle_factions_begin, factions_end),
          (is_between, ":other_faction_id", castle_factions_begin, factions_end),
          (try_begin),
  ...
  ("player_give_naked_clothes", [
    ...
    (store_random_in_range, ":boot_item", 0, 2),
    (try_begin),
      (gt, ":gender", 0),#woman
      (try_begin),
        (eq, ":head_item", 0),
        (assign, ":head_item", "itm_ladys_hood"),
      (else_try),
        (eq, ":head_item", 1),
        (assign, ":head_item", "itm_barbette"),
      (else_try),
        (eq, ":head_item", 2),
        (assign, ":head_item", "itm_wimple_a"),
      (try_end),
      (try_begin),
        (eq, ":body_item", 0),
        (assign, ":body_item", "itm_ribaude_dress"),
      (else_try),
        (eq, ":body_item", 1),
        (assign, ":body_item", "itm_peasant_dress_c"),
      (else_try),
        (eq, ":body_item", 2),
        (assign, ":body_item", "itm_maid_dress"),
      (try_end),
      (try_begin),
        (eq, ":boot_item", 0),
        (assign, ":boot_item", "itm_woolen_hose"),
      (else_try),
        (eq, ":boot_item", 1),
        (assign, ":boot_item", "itm_khergit_leather_boots"),
      (try_end),
    (else_try),
      (try_begin),
        (eq, ":head_item", 0),
        (assign, ":head_item", "itm_straw_hat"),
      (else_try),
        (eq, ":head_item", 1),
        (assign, ":head_item", "itm_leather_cap"),
      (else_try),
        (eq, ":head_item", 2),
        (assign, ":head_item", "itm_common_hood"),
      (try_end),
      (try_begin),
        (eq, ":body_item", 0),
        (assign, ":body_item", "itm_rich_tunic_e"),
      (else_try),
        (eq, ":body_item", 1),
        (assign, ":body_item", "itm_drz_kaftan"),
      (else_try),
        (eq, ":body_item", 2),
        (assign, ":body_item", "itm_shirt_b"),
      (try_end),
      (try_begin),
        (eq, ":boot_item", 0),
        (assign, ":boot_item", "itm_leather_boots"),
      (else_try),
        (eq, ":boot_item", 1),
        (assign, ":boot_item", "itm_ankle_boots"),
      (try_end),
    (try_end),
    (call_script, "script_change_armor", ":agent_id", ":head_item"),
    (player_set_slot, ":player_id", slot_player_equip_head, ":head_item"),
  ...
  ("game_receive_network_message", [ # called by the game whenever a custom network message is received, both clients and servers
    ...
    (try_begin),
      ...
      (player_get_slot, ":flood_log", ":sender_player_id", slot_player_flood_log),
      (try_begin),
        (eq, ":flood_log", -1),
        (assign, ":can_go", 0),
      (else_try),
        (val_add, ":flood_log", 1),
        (try_begin),
          (ge, ":flood_log", flood_log_limit),
          (player_set_slot, ":sender_player_id", slot_player_flood_log, -1),
          (kick_player, ":sender_player_id"),
          (assign, ":can_go", 0),
        (else_try),
          (player_set_slot, ":sender_player_id", slot_player_flood_log, ":flood_log"),
        (try_end),
      (try_end),
    (try_end),
    (try_begin),
      ...
      (try_begin),
        (str_length, ":lenght", s0),
        (gt, ":lenght", 255),
        (player_set_is_admin, ":sender_player_id", 0),
        (kick_player, ":sender_player_id"),
        (assign, ":can_go", 1),
      (try_end),
      (eq, ":can_go", 0),
      ...
      (try_begin),
        (neg^str_is_empty, s0),
        (neg^str_contains, s0, "@^"),
        (neg^str_contains, s0, "@INVALID TOKEN"),
        (neg^str_contains, s0, "@  "),
        (assign, ":can_go", 0),
      (try_end),
    (try_end),
...
##        (le, ":lenght", 255),
        (try_begin),
          ...
          (assign, reg10, ":sender_player_id"),
          (assign, reg11, ":chat_event_type"),
          (player_get_unique_id, reg12, ":sender_player_id"),
    ...
    (else_try), # handle players toggling visor
      ...
      (gt, ":helmet_1", 0), #** Helmet visor unkown id fix
    ...
    (else_try),
      (eq, ":event_type", client_event_open_inventory), #** Drop Inventory timer
      (player_is_active, ":sender_player_id"),
      (store_mission_timer_a, ":time"),
      (player_get_slot, ":last_action_time", ":sender_player_id", slot_player_last_action_time),
      (store_sub, ":interval", ":time", ":last_action_time"),
  ...
  ("initialize_scene_globals", # reset global values to initial values when starting a mission
   [
    ...
    (call_script, "script_faction_set_color_from_banner", "fac_outlaws", "mesh_banner_j05"),
    (call_script, "script_faction_set_color_from_banner", "fac_1", "mesh_banner_kingdom_f"),
    (call_script, "script_faction_set_color_from_banner", "fac_2", "mesh_banner_kingdom_b"),
    (call_script, "script_faction_set_color_from_banner", "fac_3", "mesh_banner_kingdom_a"),
    (call_script, "script_faction_set_color_from_banner", "fac_4", "mesh_banner_kingdom_d"),
    (call_script, "script_faction_set_color_from_banner", "fac_5", "mesh_banner_kingdom_e"),
    (call_script, "script_faction_set_color_from_banner", "fac_6", "mesh_banner_kingdom_c"),
    (call_script, "script_faction_set_color_from_banner", "fac_7", "mesh_banner_kingdom_g"),
    (call_script, "script_faction_set_color_from_banner", "fac_8", "mesh_banner_a03"),
  ...
  ("on_agent_spawned", [# server: set agent slots and attributes after spawning
    ...
    (agent_set_slot, ":agent_id", slot_agent_animal_spawn_instance, -1),
  ...
  ("setup_agent_for_respawn", # server: when an agent dies, clean up slots and attached props; set up lootable items
   [(store_script_param, ":agent_id", 1),
    ...
    (try_begin),
      ...
      (player_set_slot, ":player_id", slot_player_death_coord_x, 0),
      (player_set_slot, ":player_id", slot_player_death_coord_y, 0),
      (player_set_slot, ":player_id", slot_player_death_coord_z, 0),
      ...
      (try_begin),
        ...
        (try_begin),
          ...
          (player_set_slot, ":player_id", slot_player_death_coord_x, ":x"),
          (position_get_y, ":y", pos0),
          (player_set_slot, ":player_id", slot_player_death_coord_y, ":y"),
...
##        (player_set_slot, ":player_id", slot_player_equip_horse, ":item_id"),
      ...
      (else_try),
        ...
        (scene_prop_set_slot, ":corpse_instance_id", slot_scene_prop_inventory_unique_id, "$g_last_inventory_unique_id"),
        (assign, ":inventory_slot", slot_scene_prop_inventory_begin),
        (try_for_range, ":agent_slot", 4, 7),
          (agent_get_item_slot, ":item_id", ":agent_id", ":agent_slot"),
          (ge, ":item_id", all_items_begin),
      ...
      (try_end),
      (try_begin),
        (eq, "$g_game_type", "mt_permanent_death"),
   ...
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (else_try),
      ...
      (try_begin),
        (agent_get_wielded_item_slot_no, ":slot_id", ":agent_id"),
        (val_add, ":slot_id", slot_agent_item_1_value),
        (agent_slot_gt, ":agent_id", ":slot_id", 0),
        (agent_set_slot, ":agent_id", ":slot_id", 0),
        (val_sub, ":slot_id", slot_agent_item_1_value - 1),
        (agent_unequip_item, ":agent_id", ":item_id", ":slot_id"),
      (else_try),
        (call_script, "script_cf_agent_consume_item", ":agent_id", ":item_id", 1),
      (try_end),
...
#****
  ("cf_use_teleport_door", [ # server: handle agents using a door which teleports to another linked door
    ...
    (assign, reg2, ":x_offset"),
    (assign, reg3, ":y_offset"),
    (assign, reg4, ":z_offset"),
    (assign, reg5, ":is_pickable"),
    (assign, reg6, ":horse_can_tp"),
    (send_message_to_url_advanced, script_ip_address + "/check_door_key_tp<{reg0}<{reg1}<{reg2}<{reg3}<{reg4}<{reg5}<{reg6}", "@WSE2", "script_check_door_key_tp_return", "script_check_door_key_tp_fail"),
    ]),
...
scripts.extend([
  ...
  ("cf_buy_sell_item_stockpile", # server: handle players using an item or horse stockpile for buying and selling
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (else_try), # if the horse selling didn't respond with a failure message and the weapon hand is empty, try buy
      ...
      (eq, "$g_game_type", "mt_quick_battle"),
      (eq, ":weapon_item_id", -1),
      (call_script, "script_cf_buy_item", ":agent_id", ":instance_id", 0),
      (val_sub, ":stock_count", 1),
    (else_try),
      (eq, ":stock_count", 0),
      (assign, ":fail", 1),
   ...
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (try_begin),
      ...
      (try_begin),
        ...
        (multiplayer_send_3_int_to_player, ":player_id", server_event_scene_prop_set_slot, ":instance_id", slot_scene_prop_stock_count, ":stock_count"),
      (else_try),
        (call_script, "script_cf_buy_item", ":agent_id", ":instance_id", 1),
      (try_end),
    (else_try),
      (gt, ":error_string_id", 0),
  ...
  ("cf_use_rotate_door", # server: handle opening and closing a rotating door
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (assign, reg1, ":instance_id"),
    (assign, reg2, ":left"),
    (send_message_to_url_advanced, script_ip_address + "/check_door_key<{reg0}<{reg1}<{reg2}", "@WSE2", "script_check_door_key_return", "script_check_door_key_fail"),
    ]),
  ...
  ("transfer_inventory", # server: handle inventory transfers within and between scene props and their agent
   [(store_script_param, ":player_id", 1), # must be valid
    ...
    (try_begin), # failures in this section will force disconnection from the scene prop, to stop receiving updates
      ...
      (else_try),
        (scene_prop_slot_eq, ":instance_id", slot_scene_prop_personal_inventory_player_id, ":player_id"),
      (else_try),
        (assign, ":inventory_count", 0),
      (try_end),
      (gt, ":inventory_count", 0),
      ...
      (try_begin), # for failures after this point, stay connected with the scene prop, to receive updates
        ...
        (try_begin), # remove items taken from the agent's equipment
          ...
          (eq, ":equip_item_id", ":item_id"),#Log putting items in a container
          (str_store_player_username, s11, ":player_id"),
          (str_store_item_name, s12, ":item_id"),
          (assign, reg31, ":instance_id"),
            (try_begin),
        ...
        (try_begin), # when equipped on this agent, remove from inventory view of other players
          ...
          (assign, ":to_mod_slot", -1),
        (try_end),
        (try_for_players, ":other_player_id", 1), # loop over all players currently looking at the same inventory
          (player_is_active, ":other_player_id"),
          (neq, ":other_player_id", ":player_id"),
          (player_slot_eq, ":other_player_id", slot_player_accessing_instance_id, ":instance_id"),
          (try_begin),
  ...
  ("agent_hit_with_scripted_item", # server: check for scripted effects when the agent is hit by a special item; sets damage dealt when be called from ti_on_agent_hit
   [(store_script_param, ":attacked_agent_id", 1), # must be valid
    ...
    (try_begin),
      (this_or_next|eq, ":weapon_item_id", "itm_surgeon_scalpel"),
  ...
  ("check_agent_drowning", # server: check whether the agent's mouth is below water level, dealing damage after a length of time
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (try_begin),
      (set_fixed_point_multiplier, 100),#** Rewritten to optimize.
      (agent_is_alive, ":agent_id"),
      (agent_get_position, pos1, ":agent_id"),
      (position_get_z, ":height", pos1),
      (lt, ":height", 0),
   ...
   [(store_script_param, ":player_id", 1), # must be valid
    ...
    (try_begin),
      ...
      (try_begin), # the first script parameter is the name string id, which must be in the appropriate section of module_strings.py
        ...
        animation_menu_entry("str_anim_sit", animation="anim_sitting_pillow_male", woman_alt_animation="anim_sitting_pillow_female", weapon_requirement=-1
                             , prevent_if_moving=1, prevent_if_on_horse=1, upper_body_only=0),
        ...
        animation_menu_entry("str_anim_sleeping", animation="anim_sleeping", man_sound="snd_man_yawn", woman_sound="snd_man_yawn", weapon_requirement=-1, prevent_if_moving=1
                             , prevent_if_on_horse=1, upper_body_only=0),
...
scripts.extend([
  ...
  ("skybox_update", [
    ...
    (else_try),
      ...
      (try_begin),
        ...
        (display_message, "str_time_sunrise"),
      (try_end),
    (else_try),
      ...
      (try_begin),
        ...
        (display_message, "str_time_sunset"),
      (try_end),
    (else_try),
      ...
      (try_begin),
        ...
        (display_message, "str_time_night"),
      (try_end),
    (try_end),
  ]),
  ...
  ("skybox_animate_sun_and_moon", [
    ...
    (try_begin),
      ...
      (scene_prop_get_instance, ":moon", "spr_srp_skybox_moon", 0),
    (else_try),
      (display_message, "str_time_error"),
    (try_end),
