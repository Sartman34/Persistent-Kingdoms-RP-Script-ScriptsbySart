...
scripts.extend([
  ...
  ("cf_use_rotate_door", # server: handle opening and closing a rotating door
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (assign, reg2, ":left"),
    (player_get_unique_id, reg3, ":player_id"),
    (assign, reg4, ":player_id"),
    (send_message_to_url_advanced, script_ip_address + "/check_door_key<{reg0}<{reg1}<{reg2}<{reg3}<{reg4}", "@WSE2", "script_check_door_key_return", "script_check_door_key_fail"),
    ]),
